...
$smarty->assign('show_marketprice',    true);
/*------------------------------------------------------ */
//-- 高级搜索
/*------------------------------------------------------ */
...
else
{
    ...
    $_REQUEST['min_price']  = !empty($_REQUEST['min_price'])  ? intval($_REQUEST['min_price'])  : '';
    $_REQUEST['max_price']  = !empty($_REQUEST['max_price'])  ? intval($_REQUEST['max_price'])  : '';
		$_REQUEST['market_min_price']  = !empty($_REQUEST['market_min_price'])  ? intval($_REQUEST['market_min_price'])  : '';
    $_REQUEST['market_max_price']  = !empty($_REQUEST['market_max_price'])  ? intval($_REQUEST['market_max_price'])  : '';
    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'form')
    {
        ...
        $adv_value['max_price'] = $_REQUEST['max_price'];
				$adv_value['market_min_price'] = $_REQUEST['market_min_price'];
        $adv_value['market_max_price'] = $_REQUEST['market_max_price'];
...
$max_price  = $_REQUEST['max_price'] > 0? " AND g.shop_price <= '$_REQUEST[max_price]'" : '';

$market_min_price  = $_REQUEST['market_min_price'] > 0 ? " AND g.market_price >= '$_REQUEST[market_min_price]'" : '';
$market_max_price  = $_REQUEST['market_max_price'] > 0 ? " AND g.market_price <= '$_REQUEST[market_max_price]'" : '';
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        ...
        "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price .$market_min_price . $market_max_price . $intro . $outstock ." ) ".$tag_where." )";
    $count = $db->getOne($sql);
    ...
    $smarty->assign('max_price',  $max_price);
		$smarty->assign('market_min_price',  $market_min_price);
    $smarty->assign('market_max_price',  $market_max_price);
    ...
    if (isset($_REQUEST['pickout']))
    {
		...
		$url_format .= "&amp;market_min_price=" . $_REQUEST['market_min_price'] ."&amp;market_max_price=" . $_REQUEST['market_max_price'];
    ...
    $pager['search'] = array(
        ...
        'max_price'  => $_REQUEST['max_price'],
				'market_min_price'  => $_REQUEST['market_min_price'],
        'market_max_price'  => $_REQUEST['market_max_price'],
        'action'     => $action,
